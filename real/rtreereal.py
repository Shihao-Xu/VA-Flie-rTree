...
realData=np.load('real_data_30d.npy')
...
class rtree():
    ...
    def create(self):
        #lst = np.random.random(size=self.number)  # create points
        #d1 = []
        #for i in range(self.number - self.dimension+1):
        #    if i==0 or i % self.dimension==0:
        #      
        #       d1.append(point_gen(lst[i : ], self.dimension ))
        # we need to read real data here      
        d=[]
        for i in range(50000):
            d.append (tuple(realData[i][:self.dimension]))
        return d
    ...
    for i in dim:
      if i%2==0:
        ...
        for n in range(10):
            ...
            lst = np.random.random(size=tree.dimension)
